...
def index(request):
    news_objects     = news.objects.order_by("-published_date")[:1]
    headline_objects = news.objects.order_by("-published_date")[1:9]
    ...
    return render(request, 'index.html',{'leagues': League.objects.all(), 'news' : news_objects, 'headlines' : headline_objects, 'events' : events_objects, 'puzzles' : puzzles } )
